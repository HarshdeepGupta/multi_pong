/**
 * Created by hd on 14/4/16.
 */

public interface Commons {

    public static final int WIDTH = 500;
    public static final int HEIGTH = 500;
    public static final int BOTTOM_EDGE = 390;
    public static final int N_OF_BRICKS = 30;
    public static final int INIT_PADDLE1_X = 250;
    public static final int INIT_PADDLE1_Y = 480;

    public static final int INIT_PADDLE2_X = 250;
    public static final int INIT_PADDLE2_Y = 0;

    public static final int INIT_PADDLE3_X = 0;
    public static final int INIT_PADDLE3_Y = 240;

    public static final int INIT_PADDLE4_X = 480;
    public static final int INIT_PADDLE4_Y = 240;

    public static final int INIT_BALL_X = 230;
    public static final int INIT_BALL_Y = 355;
    public static final int DELAY = 1000;
    public static final int PERIOD = 10;
}
